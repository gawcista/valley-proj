...
| `valley_projection_summary[].valley_weights` | object | Raw q-cut weights keyed by configured valley-subspace label |
...
| `trusted_representation_count` | int | Count of trusted representation entries |
| `blocked_representation_count` | int | Count of blocked representation entries |
| `diagnostic_only_count` | int | Count of diagnostic-only representation entries |
...
| `subspace_hsp_little_group_operation_ids` | list[int] | Operation IDs in the valley-projected subspace HSP little group `G_k^(a)`; this is the public irrep group |
| `hsp_little_group_operation_ids` | list[int] | Legacy alias for `subspace_hsp_little_group_operation_ids`; not the parent full HSP little group |
| `valley_preserving_operation_ids` | list[int] | Alias for the valley-preserving HSP subgroup `G_k^(a)` |
| `parent_hsp_little_group_operation_ids` | list[int] | Parent full HSP little group `G_k`; provenance only, not the public irrep group |
| `valley_sewing_operation_ids` | list[int] | Parent HSP little-group operations that change the selected valley; valley-sewing provenance only |
...
| `irrep_matching.matching_status` | string | `"matched"`, `"incomplete"`, `"not_matched"`, `"blocked"`, `"identity_only_not_irrep_distinguishing"` |
| `irrep_matching.matching_strategy` | string | `"bilbao_restricted_character"` (generic restricted-character path), `"identity_only_valley_preserving_subgroup"` (identity-only `G_k^(a)`) |
| `irrep_matching.irrep_multiplicities` | object | Irrep label → integer multiplicity |
| `irrep_matching.local_representation_dimension` | int\|null | Local representation dimension `chi_a(e)` (when available, e.g. identity-only) |
| `irrep_matching.reason` | string | Optional; irrep matching note (not a blocker) |
...
| `subspace_space_group` | object | Physical subspace-space-group provenance. When present, `candidate_space_group_symbol` is the canonical subgroup identity and `subspace_group_candidate` is the derived scalar key. |
...
| `irrep_records_by_kpoint` | object | Per-kpoint trusted irrep provenance records (operation, order, matched_irrep, character, eigenphases, workflow_path, readiness_level, source, optional `irrep_source_provenance`). Only `ready_for_ebr_input=true` rows appear. |
...
Each excluded instance:

| Field | Type | Description |
|-------|------|-------------|
| `source_instance_id` | string | Source problem instance ID |
| `valley` | string | Valley label |
| `subspace_group_candidate` | string | Compact scalar group key |
| `subspace_space_group` | object | Optional physical subspace-space-group provenance |
| `status` | string | Instance status |
| `ready_for_ebr_decomposition` | bool | Decomposition readiness |
| `exclusion_reasons` | list[string] | Reasons the instance was excluded from export |
...
Two mutually exclusive user-supplied inputs are supported:

- `analysis.reduced_ebr.table_file`: load a prebuilt validated reduced EBR table
- `analysis.reduced_ebr.spec_file`: build and validate the reduced table at
  runtime from a reviewed `irreptables` mapping spec

Production reduced EBR data comes from the irreptables runtime reducer, but
ValleyScope does not ship built-in tables or built-in specs. Without either
`table_file` or `spec_file`, the output carries `status: missing_table`.
...
| `table_status` | string | `"not_applicable"` (no export bundle available), `"not_provided"` (enabled but neither `table_file` nor `spec_file` was supplied), `"loaded"` (table loaded and decomposition attempted) |
...
| `reduced_ebr_input` | object | Present when the workflow records compact reduced-EBR input provenance |

`reduced_ebr_input` uses compact non-path provenance so public outputs can
distinguish equivalent input paths without exposing local filesystem layout.
Supported `source` values are:

- `"table_file"`: loaded from `analysis.reduced_ebr.table_file`
- `"spec_file"`: built at runtime from `analysis.reduced_ebr.spec_file`
- `"not_provided"`: reduced EBR was enabled but neither input was supplied

When `source == "table_file"`, the object may contain:

| Field | Type | Description |
|-------|------|-------------|
| `table_file_stem` | string | Basename stem of the user-supplied table file |

When `source == "spec_file"`, the object may contain:

| Field | Type | Description |
|-------|------|-------------|
| `spec_file_stem` | string | Basename stem of the user-supplied spec file |
| `subspace_group_candidate` | string | Physical subspace-space-group symbol from the built reduced table |
| `data_source` | string | Runtime source provenance, currently `irreptables` |
...
| `classification` | string | `"atomic-compatible-candidate"`, `"in_integer_span_no_nonnegative_witness"`, or `"outside_integer_span"` |
...
- `in_integer_span_no_nonnegative_witness`: target is in the integer EBR span
  but no nonnegative combination exists (requires negative coefficients). An
  `integer_solution` witness with signed coefficients is provided.
  (Legacy name: `fragile-topology-candidate`.)
- `outside_integer_span`: target is outside the integer EBR span.
  (Legacy name: `stable-topology-candidate`.)
...
When `classification == "in_integer_span_no_nonnegative_witness"`, additionally:
...
| `irrep_source_provenance_by_kpoint` | object | Present when the source export bundle contains per-HSP irrep provenance records. Keys are sampled HSP labels; values are lists of compact records preserving `matched_irrep`, `irrep_multiplicity`, `source_hsp_label`, source table/spinor metadata, valley-preserving operation IDs, source table operation indices, and operation-mapping provenance where available. |
...
| `irrep_source_provenance_by_kpoint` | object\|null | Same compact per-HSP provenance shape as solution entries, included when available so excluded bundles remain auditable against the sampled HSP basis. |
...
Current ingestion-record schema version: `"1.3.0"`.
...
| Field | Type | Description |
|-------|------|-------------|
| `schema_version` | string | Ingestion-record schema version, currently `"1.3.0"` |
...
| `reduced_ebr_input` | object | Optional compact reduced-EBR input provenance copied from `valley_reduced_ebr_mapping.json` |
...
| `in_integer_span_no_nonnegative_witness` | int | Count of `in_integer_span_no_nonnegative_witness` solutions (legacy: `fragile_topology`) |
| `outside_integer_span` | int | Count of `outside_integer_span` solutions (legacy: `stable_topology`) |
  ...
  "valley_projection_summary": [
    {
      ...
      "valley_weights": {"K_valley": 0.97, "Kp_valley": 0.01},
