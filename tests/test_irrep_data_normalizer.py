...
_FAKE_EBR_DATA = {
    ...
    "ebrs": [
        ...
        {"ebr_name": "EBR_C", "wyckoff_position": "1a",
...
def test_e2e_fake_loader_to_reduced_ebr_mapping(tmp_path):
    ...
    # Build table from fake spec file + fake loader.
    ...
    assert len(table["irreps"]) == 3
    assert len(table["ebrs"]) == 3

    # Validate through standard loader.
    import tempfile, os
    tmp = tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False, encoding="utf-8")
    try:
        json.dump(table, tmp)
        tmp.close()
        validated = load_reduced_ebr_table(tmp.name)
    finally:
        os.unlink(tmp.name)
    assert validated["subspace_group_candidate"] == "C3_like"

    # Feed into reduced EBR mapper with a toy bundle.
    bundle = {
        "bundles": [{
...
def test_e2e_smoke_no_material_names():
    """E2E smoke test fixtures must not contain material names."""
    for name in ["tMoTe2", "tZrSe2", "MoTe2", "ZrSe2"]:
        assert name not in json.dumps(_FAKE_EBR_DATA)
        assert name not in json.dumps(_FAKE_SPEC)


def test_spec_doc_exists_and_covers_keys():
    """Canonical spec doc must exist and document required keys."""
    doc = Path("docs/build_reduced_ebr_table_spec.md")
    assert doc.exists()
    text = doc.read_text(encoding="utf-8")
    for key in ["schema_version", "data_source", "space_group_number", "spinful",
                "source_hsp_by_irrep", "valleyscope_key_by_source_irrep",
                "expected_hsps", "allowed_irrep_keys", "subspace_group_candidate"]:
        assert key in text, f"spec doc must document '{key}'"
    assert "valleyscope build-reduced-ebr-table" in text
