...
# -----------------------------------------------------------------------
# build-reduced-ebr-table CLI + spec-file helper
# -----------------------------------------------------------------------
...
_P150_SPEC = {
    ...
    "expected_hsps": ["GammaM", "KM", "A", "H", "HA", "L", "M"],
    "allowed_irrep_keys": [],
    "subspace_group_candidate": "C3_like",
}
# Build key map: hsp:label for each source irrep label.
for _label, _hsp in _P150_SPEC["source_hsp_by_irrep"].items():
    _P150_SPEC["valleyscope_key_by_source_irrep"][_label] = f"{_hsp}:{_label.lstrip('-')}"
_P150_SPEC["allowed_irrep_keys"] = list(_P150_SPEC["valleyscope_key_by_source_irrep"].values())


def test_spec_file_helper_produces_validated_table(tmp_path):
    """build_reduced_table_from_spec_file reads spec and produces validated table."""
    from valleyscope.analysis.irreptables_runtime_table_builder import (
        build_reduced_table_from_spec_file,
    )
    spec_path = tmp_path / "spec.json"
    spec_path.write_text(json.dumps(_P150_SPEC), encoding="utf-8")
    table = build_reduced_table_from_spec_file(str(spec_path))
    assert table["subspace_group_candidate"] == "C3_like"
    assert len(table["ebrs"]) > 0


def test_spec_file_missing_keys_raises(tmp_path):
    """Missing required spec keys raise ValueError."""
    from valleyscope.analysis.irreptables_runtime_table_builder import (
        build_reduced_table_from_spec_file,
    )
    spec_path = tmp_path / "spec.json"
    spec_path.write_text(json.dumps({"sg_number": 150}), encoding="utf-8")
    with pytest.raises(ValueError, match="missing required keys"):
        build_reduced_table_from_spec_file(str(spec_path))


def test_cli_build_reduced_ebr_table_writes_and_validates(tmp_path, capsys):
    """CLI writes a table that passes load_reduced_ebr_table."""
    ...
    assert "C3_like" in captured
    assert "reduced EBR table:" in captured


def test_cli_missing_spec_fails(tmp_path):
    """CLI fails clearly on missing spec file."""
    from valleyscope.cli import main
    rc = main(["build-reduced-ebr-table", str(tmp_path / "nonexistent.json")])
    assert rc != 0


def test_cli_no_material_names():
    """CLI build-reduced-ebr-table code must not contain material names."""
    src = Path("valleyscope/cli.py").read_text(encoding="utf-8")
    for name in ["tMoTe2", "tZrSe2", "MoTe2", "ZrSe2"]:
        assert name not in src, f"cli.py must not contain {name!r}"


def test_builder_helper_no_material_names():
    src = Path(
