...
`valley-proj` analyzes VASP-derived moire wavefunctions at selected high-symmetry points. It constructs momentum-space projectors onto user-defined monolayer valley sectors, diagnoses whether a target state or subspace is valley-resolved, and computes symmetry representation data needed for rotation-eigenvalue analysis.
...
Thus \(W_\times\) is a numerical diagnostic of the valley-window definition, not a new condensed-matter observable. In prose, use "cross-sector overlap weight"; `ambiguous_weight` is the current serialized output key.
...
`valley-proj` 用于分析 VASP 计算得到的 moire 超胞波函数。它在给定高对称点处构造面向 monolayer valley sectors 的动量空间投影算符，诊断目标态或目标子空间是否可以被可靠地分辨为 valley 态，并输出旋转本征值分析所需的对称性表示矩阵。
...
因此，\(W_\times\) 是对 valley-window 选择的数值诊断，不是新的凝聚态物理可观测量。自然语言中建议称为“cross-sector overlap weight”或“跨 sector 重叠权重”；`ambiguous_weight` 只是当前输出文件中的字段名。
